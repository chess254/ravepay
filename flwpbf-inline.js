...
!(function (e, t) {
  ...
  function (e) {
    ...
    e.__Base64__ = Object.freeze(
      Object.create(null, {
        encode: {
          value: function (e) {
            return (function (e) {
              for (
                var n, r, o, c, i = "", a = 0, d = (e = String(e)).length % 3;
                a < e.length;
                ...
                if (
                  ...
                  (c = e.charCodeAt(a++)) > 255
                )
                  throw new TypeError(
                    "Failed to execute 'btoa' on 'Window': The string to be encoded contains characters outside of the Latin1 range."
                  );
                i +=
                  t.charAt(((n = (r << 16) | (o << 8) | c) >> 18) & 63) +
                  t.charAt((n >> 12) & 63) +
                  t.charAt((n >> 6) & 63) +
                  t.charAt(63 & n);
              }
            ...
            return decodeURIComponent(
              escape(
                (function (e) {
                  ...
                  for (var r, o, c, i = "", a = 0; a < e.length; )
                    (r =
                      (t.indexOf(e.charAt(a++)) << 18) |
                      (t.indexOf(e.charAt(a++)) << 12) |
                      ((o = t.indexOf(e.charAt(a++))) << 6) |
                      (c = t.indexOf(e.charAt(a++)))),
                      (i +=
                        64 === o
...
(function (window) {
      ...
      try {
        if (config.PBFPubKey) {
          ...
          if (
            ...
            publicKeyUpgradesMap[merchantKey] = fetch(
              API_BASE_URL + "/v2/checkout/upgrade",
              {
                method: "POST",
              ...
              .then(function _getUpgradeRequirement(response) {
                if (response.status === "success") {
                  publicKeyUpgradesMap[merchantKey] = [!!response.data.upgrade];
  ...
  function closePopup(fromCloseMethod, cancelled) {
    try {
      w.removeEventListener("message", publicKeyHandlersMap[latestPublicKey]);
  ...
  message_handlers.cannotloadiframe = function (d) {
    ...
    document.location =
      modalframesource.src + generateQueryString(data) || document.location;
  };
  ...
  function addQueryToIframe(data, callback) {
    ...
    if (
      window.location.href.includes(
        API_BASE_URL + "/flwv3-pug/getpaidx/api/hosted_pay/"
      )
    ) {
      ...
      if (!readyToReceive) {
        ...
        modalframesource.src =
          getIframeDomain() + "/?" + generateQueryString(_data);
  ...
  function logCallbackEvent(data, redirects) {
    ...
    var tx =
      ...
      (data.data && data.data.tx) ||
      data.data.transactionobject;
            ...
            iframeData.subaccounts = [
              {
                ...
                transaction_split_ratio:
                  iframeData.subaccounts_transaction_split_ratio,
                transaction_charge_type:
                  iframeData.subaccounts_transaction_charge_type,
  ...
  function _getpaidSetup(config) {
    ...
    if (config.hosted_payment == 1 && !config.is_hosted_page) {
      ...
      form.setAttribute("action", V2_BASE_URL + "/hosted/pay");
      console.log(config);
      for (var c in config) {
        if (c === "meta") {
          ...
          c === "theme" ||
          (c === "billing" &&
            Object.prototype.toString.call(config[c]) === "[object Object]")
        ) {
          ...
          if (!~["string", "number", "boolean"].indexOf(_typeof(config[c])))
            continue;
          var i = document.createElement("input");
          i.setAttribute("type", "hidden");
  ...
  window.getpaidSetup = function (config) {
    ...
    _initialized.then(function () {
      if (config.PBFPubKey === latestPublicKey && _closed !== true) {
        ...
        w.addEventListener("message", publicKeyHandlersMap[latestPublicKey]);
